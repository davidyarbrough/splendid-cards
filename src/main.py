...
def print_game_state(game_state, current_player=None, verbose=False):
    ...
    # Print card rivers in compact rows
    print("\nLevel 3 Cards:")
...
def _print_card_row(game_state, river, verbose):
    """Print a row of cards in a compact format with colors.
    ...
    for idx, card_idx in enumerate(river):
...
def _print_card_details(game_state, card_idx, verbose):
    ...
    # Add colors to cost string
    cost_parts = []
    for color, count in card_cost.items():
        if count > 0:
            color_code = Colors.get_color_code(color)
            cost_parts.append(f"{color_code}{color.name}{Colors.RESET}:{count}")
    
    cost_str = ", ".join(cost_parts)
    
    # Add color to card color name
    color_code = Colors.get_color_code(card_color) if card_color else Colors.RESET
    colored_name = f"{color_code}{card_color.name if card_color else 'Unknown'}{Colors.RESET}"
    
    # Bold the points
    points_str = f"{Colors.BOLD}{card_points}{Colors.RESET} pts"
    
    details = f"  {colored_name} | {points_str} | Cost: {cost_str}"
    print(details)
